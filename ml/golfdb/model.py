...
class EventDetector(nn.Module):
    ...
    def init_hidden(self, batch_size, device):
        n = 2*self.lstm_layers if self.bidirectional else self.lstm_layers
        return (Variable(torch.zeros(n, batch_size, self.lstm_hidden, device=device), requires_grad=True),
                Variable(torch.zeros(n, batch_size, self.lstm_hidden, device=device), requires_grad=True))
